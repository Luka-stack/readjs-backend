...
export default class CreateData implements Seeder {
  public async run(_: any, connection: Connection): Promise<any> {
    ...
    const takahiro = await User.findOne({ username: "takahiro" });
    const tzuyu = await User.findOne({ username: "tzuyu" });
    ...
    await connection
      ...
      .into(Vote)
      .values([
        { value: 1, user: takahiro, post: post9 },
        { value: 1, user: tzuyu, post: post9 },
        { value: 1, user: tzuyu, post: post8 },
        { value: 1, user: takahiro, comment: comment1 },
        { value: 1, user: tzuyu, comment: comment1 },
        { value: 1, user: takahiro, comment: comment2 },
      ])
      .execute();
  }
}
